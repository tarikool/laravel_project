        ...
        @if( $comments )
            ...
            @foreach( $comments as $comment )
                <tbody>
                    <tr>
                        <td>{{ $comment->id }}</td>
                        <td><img height="38" src="{{$comment->photo ? $comment->photo->file : 'http://placehold.it/400x400' }}" alt=""><p>{{' '. $comment->author }}</p></td>
                        <td>{{ str_limit($comment->body,10) }}<br><p style="color: #5e5e5e; font-size: 62%;">{{ $comment->created_at->diffForHumans() }}</p></td>
                        <td>{{ $comment->email }}</td>
